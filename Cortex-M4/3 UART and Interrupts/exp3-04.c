...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		if ((uart_receive_char >= '0' && uart_receive_char <= '9') || (uart_receive_char >= 'A' && uart_receive_char <= 'Z') || uart_receive_char == '+' || uart_receive_char == '-' || uart_receive_char == ':')
...
void processCommand(int8_t* str)
{
	...
	while (h1 >= 60)
		m1 -= 60;
	UARTCharPutNonBlocking(UART0_BASE,h1/10 + '0');
	UARTCharPutNonBlocking(UART0_BASE,h1%10 + '0');
	UARTCharPutNonBlocking(UART0_BASE,':');
	UARTCharPutNonBlocking(UART0_BASE,m1/10 + '0');
	UARTCharPutNonBlocking(UART0_BASE,m1%10 + '0');
}
