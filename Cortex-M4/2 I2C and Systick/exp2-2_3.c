...
volatile uint8_t result,cnt=1,key_value;
volatile uint8_t rightshift = 0x3;
...
int main(void)
{
   ...
   //ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_25MHZ |SYSCTL_OSC_MAIN | SYSCTL_USE_PLL |SYSCTL_CFG_VCO_480), 120000000);	
	...
	{
		two[0] = 1 << (cnt - 1);
		two[1] = ((two[0] << 1) | (two[0] >> 7)) & 0xff;
		result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~rightshift);	
		for (i=0; i < (uint8_t)20; ++i)
		{
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[cnt]);
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,two[0]);
			Delay(40000);
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[cnt+1]);
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,two[1]);
			Delay(40000);
		}
		...
		{
			cnt = (cnt & 0x7) + 1;
			rightshift= ((rightshift << 1) | (rightshift >> 7)) & 0xff;
		}
	}
}
